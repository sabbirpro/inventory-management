...
def list(request):
    ...
    try:
        model = PurchasePayment.objects.all()
        serializer = PurchasePaymentListSerializer(
...
def single(request, id):
    ...
    try:
        model = PurchasePayment.objects.get(pk=id)
...
def edit(request, id):
    ...
    try:
        model = PurchasePayment.objects.get(pk=id)
        serializer = PurchasePaymentSerializer(model, data=request.data, context={
